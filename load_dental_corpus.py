#!/usr/bin/env python3
"""
Script to load all dental corpus QA pairs into the knowledge base
"""
...
def load_dental_corpus():
    """Load all dental corpus QA pairs"""
    print(f"Loading {len(dental_corpus)} dental corpus QA pairs...")
    
    success_count = 0
    error_count = 0
    
    for i, qa_pair in enumerate(dental_corpus, 1):
        try:
            ...
            if response.status_code == 200:
                success_count += 1
                print(f"✅ {i:2d}/30: {qa_pair['question'][:50]}...")
            else:
                error_count += 1
                print(f"❌ {i:2d}/30: Failed - {qa_pair['question'][:50]}... (Status: {response.status_code})")
                
        except Exception as e:
            error_count += 1
            print(f"❌ {i:2d}/30: Error - {qa_pair['question'][:50]}... ({str(e)})")
    ...
    try:
        rebuild_response = requests.post(f"{API_BASE_URL}/api/chatbot/system/rebuild-index", timeout=30)
        if rebuild_response.status_code == 200:
            print("✅ RAG index rebuilt successfully")
        else:
            print(f"❌ Failed to rebuild RAG index (Status: {rebuild_response.status_code})")
    except Exception as e:
